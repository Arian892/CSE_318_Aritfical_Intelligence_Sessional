...
int Hamming_Distance (vector <vector<int>> &grid){
    int k = grid[0].size ();
    int count = 0 ; 
...
int Linear_conflict (vector <vector<int>> &grid){
    ...
    return (2*count_Linear_conflict(grid) + Manhattan_Distance(grid) ); // add the manhattan distance to the linear conflict
}
